...
This is an AI system that had to decide where AI belongs. A model reads language here — column headers, bank narrations, questions about a result — and proposes what to do with a settlement the engine withheld. Every one of those outputs is checked in code before it counts: a narration field is kept only if it appears in the narration, an investigator's proposal has to pass a verifier, an answer has to trace every figure to the record. And none of them decides which transactions compose a settlement, because that is a money question with millions of arithmetically valid wrong answers, and I measured what happens when you let a search guess at that: zero percent. I measured the model uses too, and I'll tell you the one I threw away — the first investigator run scored 46% because the model was reading the benchmark's labels; with them hidden, the cases built whole and a retry that is told why it failed, it is 38%, and I publish what that means: 20 of its 22 are cases where the right answer is to ask for a missing record, and on the solvable ones it mostly escalates rather than guess. The honest version of the data-handling story too, because judges check: when these are switched on they send real data out — sample values for schema mapping, narrations, the case of a withheld settlement, the recorded result for Q&A — fenced as untrusted content the model can only read, never act on. It is not "nothing ever leaves the server," and I'd rather tell you that myself than have you find the gap.
