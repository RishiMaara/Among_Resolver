...
The core pipeline is covered by 769 rigorous backend tests, from file parsing hazards to time-zone misalignment, cross-feed identity collisions, un-anchored math fallbacks, and — added after a 1,000-scenario adversarial sweep found it — a settlement's anchored leg being assigned to a different settlement in the joint N:M path.
...
============================ 769 passed in 96.2s =============================
```
**Result:** ✅ 769 passed, 0 failed — screening the UN Consolidated List (3,422 identifiers).
...
clone runs all 769. Its retrieval date is in its header and `/health` reports
