...
def ai_status():
    ...
    return {
        ...
        "fallbacks": llm_provider.FALLBACK_MODELS if live else [],
        "live": live,
        "uses": {
        ...
        "plain": (
            f"{llm_provider.DEFAULT_MODEL} is live on this server (with "
            f"{', '.join(llm_provider.FALLBACK_MODELS) or 'no fallback'} when it is overloaded), "
            f"metered per visitor and per day. Every answer it gives is checked in code before it is used, and none "
